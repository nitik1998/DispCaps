...
class Encoder_Decoder(nn.Module):
    def __init__(self,batch_norm=True,in_channels=3,out_channels=1,planes=64,num_caps=8,caps_size=3,num_of_levels=4,no_fac=1):
        ...
        self.conv0 = conv_elu(batch_norm,3,64,3,1,1)
        ...
        self.conv1 = conv_elu(batch_norm,64,128,3,1,1)
        self.res1=residual_block(in_planes=128)
        self.resolve1= resolve_layer(256+4,128)
        ...
        self.resolve2=resolve_layer(512+4,256)
        ...
        self.conv3=conv_elu(batch_norm,256,256,7,1,pad=3)
        self.res3=residual_block(in_planes=256)
        self.resolve3=resolve_layer(512+4,256)
    ...
    def forward(self,x,y):
        ...
        out_y1=self.res0(self.conv0(y))

        out_1=torch.cat((out_x1,out_y1),1)
        out1=self.resolve0(out_1)
        out_a1,out_p1=self.conv_a0(out1),self.conv_p0(out1)
        out_a1=torch.sigmoid(self.bn_a0(out_a1))
       
        out_p1=self.bn_p0(out_p1)

        out_a2,out_p2=self.caps0(out_a1,out_p1)
        out_p2=self.bn_p1(out_p2)

        ## start of conv 2
        out_x2=self.res1(self.conv1(out_x1))
        out_y2=self.res1(self.conv1(out_y1))

        out_2=torch.cat((out_x2,out_y2),1)
        out_2= torch.cat((out_2,out_a2),1)
        out_2=self.resolve1(out_2)
        out_a2=self.conv_a1(out_2)
        out_a2=torch.sigmoid(self.bn_a1(out_a2))
        
        out_a3,out_p3=self.caps1(out_a2,out_p2)
        out_p3=self.bn_p2(out_p3)

        ## star of conv 3

        out_x3=self.res2(self.conv2(out_x2))
        out_y3=self.res2(self.conv2(out_y2))

        out_3=torch.cat((out_x3,out_y3),1)
        out_3= torch.cat((out_3,out_a3),1)
        out_3=self.resolve2(out_3)
        out_a3=self.conv_a2(out_3)
        out_a3=torch.sigmoid(self.bn_a2(out_a3))
        
        out_a4,out_p4=self.caps2(out_a3,out_p3)
        out_p4=self.bn_p2(out_p4)

        ## start of conv 4##

        out_x4=self.res3(self.conv3(out_x3))
        out_y4=self.res3(self.conv3(out_y3))

        out_4=torch.cat((out_x4,out_y4),1)
        out_4= torch.cat((out_4,out_a4),1)
        out_4=self.resolve3(out_4)
        out_a4=self.conv_a3(out_4)
        out_a4=torch.sigmoid(self.bn_a3(out_a4))
        out_a5,_=self.caps3(out_a4,out_p4)
